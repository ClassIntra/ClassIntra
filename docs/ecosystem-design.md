...
## 5.5 体验品质规范（德芙般流畅 + 视觉一致）

> **产品价值（用户明确要求）**：ClassIntra 注重用户使用体验，以及「德芙般的流畅」与视觉体验。
> 本节把这条要求**翻译成可验证的硬指标**。规范一旦成立，修复与审核都以此为准。

### 5.5.0 视觉基线：iOS 为主，鸿蒙为参照

**基线声明（用户确认）**：视觉体验**以 iOS 视觉体验为主基线**，**兼容华为鸿蒙（HarmonyOS）的视觉规范作为可接受对标**。

**为什么这两者可以共存**：iOS HIG 与 HarmonyOS Design 在视觉语言的核心层高度收敛，且 ClassIntra 已有令牌体系与之天然对齐：

| 维度 | iOS HIG | HarmonyOS Design | ClassIntra 令牌 | 对齐结论 |
|---|---|---|---|---|
| 圆角层级 | 连续圆角（Continuous Corner） | 分层圆角 | `--radius-xs`→`--radius-3xl` | ✅ 已对齐（8 档） |
| 大标题排版 | Large Title 递减层次 | 标题分层 | `--font-size-largeTitle`→`caption2` | ✅ 已对齐（10 档） |
| 卡片式分组 | Inset Grouped | 分组卡片 | `.card` + `--card-bg` | ✅ 已对齐 |
| 缓冲动效 | 弹簧/减速曲线 | 曲线动效 | `--ease-standard/decelerate/spring` | ✅ 已对齐 |
| 层次纵深 | 半透明 + 模糊 | 半透明 + 模糊 | `--glass-blur-*` | ✅ 已对齐 |
| 强调色 | System Colors | 品牌色 | `--primary-color` | ✅ 已对齐 |

**因此策略是：以 iOS 令牌为准绳，鸿蒙的差异点作为「可选适配层」而非「第二套体系」。** 理由有三：

1. **令牌体系只有一套**。增加第二套视觉体系会直接违背「体验不割裂」的核心诉求——同一系统内出现两种视觉语言，比任何一种都不统一更糟；
2. **鸿蒙与 iOS 的差异集中在细节**（如圆角连续性的数学实现、动效曲线的缓动程度），**不在结构**。这些差异可以通过**令牌值替换**吸收，无需新建设计系统；
3. **目标设备是 Android 平板 + 腾讯 X5 / Chromium 89**。真机渲染能力有限，两套体系的收益会被设备的渲染折损吃掉大半。

**可吸收的鸿蒙差异点（记入待办，按需适配）**：

| 鸿蒙特征 | 与 iOS 的差异 | 吸收方式 |
|---|---|---|
| 圆角连续性 | 鸿蒙偏「几何圆角」，iOS 是「连续曲率」 | CSS 无法表达连续曲率，**统一为几何圆角**（现行做法正确） |
| 动效曲线 | 鸿蒙曲线更「快进慢出」 | 通过 `--ease-standard` 的值替换吸收，不新增令牌 |
| 毛玻璃强度 | 鸿蒙的模糊半径普遍更小 | 通过 `--glass-blur-*` 的值替换吸收 |
| 大标题字号 | 鸿蒙字阶比 iOS 略小 | 通过 `--font-size-largeTitle` 的值替换吸收 |

**结论**：规范层**不引入鸿蒙专用令牌**；若未来确需适配，走**主题包**（`themes/*`，`type: light/dark`）覆盖令牌值，而非改核心。

### 5.5.1 动效时长与曲线映射（唯一权威表）

**问题**：动效总开关（`--ease-standard` 单档 + `[data-no-motion]` 全局降级）是健全的，但**开启状态下没有任何约束**。实测发现同一系统内混用 `ease`（CSS 默认贝塞尔）与 `cubic-bezier(0.25,0.1,0.25,1)`，时长横跨 `0.05s` 到 `0.5s`，且 `global.scss` 内存在 `0.3s` 与令牌 `0.25s` 的漂移。

用户感受不到具体曲线的数值，但会感受到「有的地方灵动、有的地方发木」——**这是对视听一致性最真实的威胁**。

**规范**：任何过渡必须从下表取值，**禁止硬编码曲线与时长**。

| # | 交互类型 | 曲线令牌 | 时长令牌 | 实际值 | 说明 |
|---|---|---|---|---|---|
| 1 | 按压反馈（按钮/列表项/导航按钮） | `--ease-standard` | `--duration-fast` | 0.15s | 用户手指按下后到达稳态的时间，**必须 ≤ 0.15s**，否则手感「粘」 |
| 2 | 状态切换（Tab 选中、开关切换） | `--ease-standard` | `--duration-fast` | 0.15s | 与按压反馈同档，保持节奏统一 |
| 3 | 悬停/聚焦（桌面端） | `--ease-standard` | `--duration-fast` | 0.15s | |
| 4 | 容器展开/收拢（折叠面板、抽屉） | `--ease-emphasized` | `--duration-normal` | 0.25s | 强调曲线，有力度但不突兀 |
...
1. **禁止 `ease` / `linear` / `ease-in-out` 等 CSS 关键字**（循环动画的 `linear` 除外），必须是上表令牌；
2. **禁止直接写秒数**，必须走 `--duration-*`；
3. **退出时长 ≤ 进入时长**（一般取进入的 0.6 倍，即 0.25s → 0.15s）。
...
浏览器渲染流水线中，只有 `transform` 与 `opacity` 可以跳过布局（Layout）与绘制（Paint），直接在合成器（Compositor）上完成。

| 属性 | 是否可过渡 | 说明 |
|---|---|---|
| `transform` | ✅ 首选 | 位移、缩放、旋转一律用它 |
| `opacity` | ✅ 首选 | 淡入淡出用它 |
| `filter` | ⚠️ 谨慎 | 会触发重绘，仅在必要时用，且避免大面积 |
| `backdrop-filter` | ⚠️ 高开销 | 见 §5.5.3 |
| `box-shadow` | ⚠️ 高开销 | 每帧重绘。**优先用 `transform: translateY()` 模拟「抬起」** |
| `width`/`height`/`top`/`left`/`margin`/`padding` | ❌ 禁止 | 触发布局抖动（Reflow），会连带重排兄弟节点 |
| `background-position` | ❌ 禁止 | 触发重绘 |

**豁免规则（形状形变）**：当**尺寸或形状本身就是视觉语义**时，允许过渡布局属性。判据有两条，**必须同时满足**：

1. `transform: scale()` 无法还原该效果（如圆→方的 `border-radius` 变化、pill 圆角被拉伸变形）；
2. 元素面积足够小（**< 200px²**），重排代价可忽略。

**当前已登记的豁免（2 处，代码内均有注释说明）**：

| 位置 | 效果 | 为何不能用 transform |
|---|---|---|
| `RecordModal.vue` `.vf-btn-inner` | 录音按钮圆形 56px ↔ 方形 24px | `scale()` 会让圆角一直保持圆形，无法表达圆→方 |
| `DesktopPageIndicator.vue` `.page-dot` | 页面指示点 6px ↔ 18px 长条 | `scaleX(3)` 会把 pill 圆角拉成椭圆 |

**存量统计**：`box-shadow` 过渡 9 处、布局属性过渡 2 处（均已按上述规则评估，2 处为登记豁免）。

### 5.5.3 毛玻璃使用策略（`backdrop-filter`）

**问题**：全仓 90 处 `backdrop-filter`。目标设备为 Android 平板 + 腾讯 X5 / Chromium 89，**大面积 `backdrop-filter` 会触发全屏重采样，是帧率杀手**，且无法通过 `will-change` 优化（其代价是显存）。
...
| 层级 | 允许 | 说明 |
|---|---|---|
| 导航栏 / 状态栏 | ✅ 允许 | 面积小，且常驻，收益 > 代价 |
| 侧边栏 / 抽屉 | ✅ 允许 | 同上 |
| 模态 / 弹出层背景 | ✅ 允许 | 出现频率低，且短暂 |
| 卡片 `.card` | ⚠️ 限制 | **单屏同时可见的毛玻璃卡片 ≤ 3 个** |
| 列表项 | ❌ 禁止 | 滚动时每帧重采样，**必然掉帧** |
| 大面区背景层 | ❌ 禁止 | 用纯色或渐变替代 |
...
```
第 1 层：设备能力探测
  ├─ 低端设备（内存/核心数/UA 判定）→ documentElement[data-perf="low"]
  └─ 全局降级：backdrop-filter → 半透明纯色（保留视觉近似）

第 2 层：滚动状态感知
  ├─ 滚动开始 → documentElement[data-scrolling="1"]
  └─ 滚动期间毛玻璃全部降为半透明纯色；停止 120ms 后恢复

第 3 层：用户显式开关（沿用既有模式）
  └─ [data-no-motion="true"] 时同样降级毛玻璃（省电优先）
```

**关键设计**：降级**不改变布局**，只改变视觉质感——把 `backdrop-filter: blur(20px)` 替换为等价的 `background-color: rgba(...)`。因此不会引起任何布局抖动，降级/恢复过程中用户只感到「变清爽了」。

### 5.5.4 性能预算（可测量的验收标准）

| 指标 | 目标 | 测量方式 |
|---|---|---|
| 首屏可交互（TTI） | ≤ 2.0s | 局域网环境，平板实测 |
| 视图切换（路由跳转） | ≤ 300ms 出现内容 | 从点击到首帧内容可见 |
| 应用挂载（第三方） | ≤ 500ms | `mount()` 到首次 paint |
| 交互响应（点击 → 视觉反馈） | ≤ 100ms | 感知阈值，用户视为「即时」 |
| 滚动帧率 | ≥ 50fps | 长列表滚动期间 |
| 动画帧率 | ≥ 55fps | 过渡/弹窗期间 |
| 令牌注入 | ≤ 5ms | 单次 `inject()` 耗时 |
| 主题切换 | ≤ 100ms | 切换后到达稳定视觉状态 |

**测量工具**：构建内置诊断页（`/dev/perf`），提供 FPS 计数器、Long Task 观察器、令牌注入耗时打点。第四期交付。

### 5.5.5 令牌注入开销约束

**现状**：`:`root`` 内 147 个自定义属性，令牌注入白名单 + 前缀采集后单次注入约 140+ 个变量，逐个 `style.setProperty`（已做主题级缓存，缓存键 `data-theme|data-no-motion`）。

**约束**：
1. 注入必须发生在**容器挂载的同一帧内**（`AppShell.mounted` 同步注入），避免首帧无令牌导致的闪烁；
2. 主题切换时的重注入必须在 **100ms 内**完成（对应 §5.5.4）；
3. 若单次注入实测 > 5ms，降级策略为：只注入第三方**实际引用到的令牌**（通过扫描 CSS 文本或运行时 `getComputedStyle` 反查）。

**待优化点**：当前 `collectTokens()` 存在一个可优化项——前缀采集的内层循环对每个前缀都遍历一遍 `computed.length`。当 `TOKEN_PREFIXES` 为 2 时即遍历 2 次全量，且直接读 `style.setProperty` 属于写操作。建议改为**单次遍历 + 前缀匹配**，减少一半的枚举开销。

### 5.5.6 视觉一致性检查清单（提交前自查）

面向官方应用与第三方，逐项勾选：

- [ ] 所有颜色取自令牌（`--primary-color` / `--text-primary` …），**零硬编码 hex**
- [ ] 所有圆角取自 `--radius-*`
- [ ] 所有间距取自 `--spacing-*`
- [ ] 所有字号取自 `--font-size-*`
- [ ] 所有过渡取自 §5.5.1 映射表，无硬编码曲线/时长
- [ ] 退出时长 ≤ 进入时长
- [ ] 无 `transition: all`
- [ ] 无布局属性过渡（`width`/`height`/`top`/`left`/`margin`/`padding`）
- [ ] 毛玻璃使用符合 §5.5.3 层级表
- [ ] 在深色主题下验证过（颜色无写死、对比度足够）
