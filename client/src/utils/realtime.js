// 面向第三方应用的全局 HTTP 实时事件客户端。
// 不依赖 Chat 应用，也不要求浏览器支持 WebSocket。
...
RealtimeClient.prototype.connect = function() {
  ...
  fetch('/api/realtime/poll/register', {
    method: 'POST', headers: self._headers(), credentials: 'same-origin', body: '{}'
  }).then(function(response) {
    if (!response.ok) throw new Error('realtime register failed: ' + response.status);
    ...
    if (!self.stopped) self.pollTimer = setTimeout(function() { self.connect(); }, 5000);
  });
};
...
RealtimeClient.prototype._poll = function() {
  ...
  self.inFlight = true;
  fetch('/api/realtime/poll?since=' + encodeURIComponent(self.lastTs), {
    method: 'GET', headers: self._headers(), credentials: 'same-origin'
  }).then(function(response) {
    ...
    self.inFlight = false;
    if (!self.stopped) self.pollTimer = setTimeout(function() { self._poll(); }, 100);
  });
};
...
RealtimeClient.prototype.publish = function(event, payload, appName) {
  ...
  return fetch('/api/realtime/publish', {
    method: 'POST', headers: self._headers(), credentials: 'same-origin',
...
RealtimeClient.prototype.subscribe = function(event, handler) {
  return this.on('extension_event', function(payload) {
    if (payload && payload.event === event) handler(payload.payload, payload);
  });
};
...
RealtimeClient.prototype.disconnect = function() {
  ...
  self.pollTimer = null;
  fetch('/api/realtime/poll/unregister', {
    method: 'POST', headers: self._headers(), credentials: 'same-origin', body: '{}'
  }).catch(function() {});
};
